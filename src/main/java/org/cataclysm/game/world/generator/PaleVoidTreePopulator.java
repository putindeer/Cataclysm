...
public class PaleVoidTreePopulator extends BlockPopulator {
    ...
    public void populate(@NotNull WorldInfo worldInfo, @NotNull Random random, int chunkX, int chunkZ, @NotNull LimitedRegion limitedRegion) {
        ...
        if (!limitedRegion.isInRegion(new Location(Bukkit.getWorld("pale_void"), X, Y, Z))) {
            return;
        }

        while (limitedRegion.getType(X, Y, Z) != Material.AIR) Y++;

        Location firstTreeLocation = new Location(Bukkit.getWorld("pale_void"), X, Y, Z);
        ...
        if (startMaterial == Material.GRASS_BLOCK || startMaterial == Material.PALE_MOSS_BLOCK) {

            generatePaleTrees(limitedRegion, firstTreeLocation, random);
            ...
            for (int i = 1; i < 6; i++) {
                ...
                Location newTreeLocation = new Location(Bukkit.getWorld("pale_void"), newX, newY, newZ);

                if (!limitedRegion.isInRegion(newTreeLocation)) {
                    continue;
                }

                while (newY > 0 && limitedRegion.getType(newX, newY, newZ) == Material.AIR) {
                    newY--;
                }
                newY++;
                ...
                if (groundMaterial == Material.GRASS_BLOCK || groundMaterial == Material.PALE_MOSS_BLOCK) {
                    generatePaleTrees(limitedRegion, newTreeLocation, random);
                }
            }
        }
    }
    ...
    public void generatePaleTrees(@NotNull LimitedRegion region, Location location, @NotNull Random random) {
        ...
        region.generateTree(location, random, type, blockState -> {
            ...
            if (blockType.name().toUpperCase().contains("PODZOL")) blockState.setType(Material.PALE_MOSS_BLOCK);
