...
public class PantheonRageManager {
    private final PantheonRage rage;
    private ScheduledFuture<?> currentTask;

    public PantheonRageManager(PantheonRage rage) {
        this.rage = rage;
    }

    public void operateCurrentRage(double operation) {
        double currentRage = this.rage.current;
        double currentPhase = this.rage.getHydra().phaseManager.getPhase();
    ...
    public void setOverraged(boolean overraged) {
        ...
        this.rage.getHydra().getArena().getPlayersInArena().forEach(fighter -> {
            if (overraged) {
                this.rage.getHydra().phaseManager.updateModel(5);
                ...
                this.rage.getHydra().phaseManager.updateModel(6);
