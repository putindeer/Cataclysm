...
public class PantheonRageBar {
    ...
    public void tick(double percentage) {
        ...
        if (!this.rage.getHydra().isOutraged()) this.bossBar.progress((float) percentage);
    }

    private @NotNull Component getRageBarName(double percentage) {
        String display = !this.rage.getHydra().isOutraged()
                ? "<#50345e>ꜰᴜʀɪᴀ" : "<gradient:#581787:#8d55b5>ᴏᴜᴛʀᴀɢᴇᴅ</gradient>";

        String fire = this.interpolateHex("#fcfcfc", "#581787", percentage * 0.01) + "\uD83D\uDD25";
        return MiniMessage.miniMessage().deserialize(fire + " " + display + ": <#725182>" + this.rage.getCurrent()/100 + "<#50345e>% " + fire);
    }
