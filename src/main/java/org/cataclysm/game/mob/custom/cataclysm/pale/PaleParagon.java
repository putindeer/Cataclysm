...
public class PaleParagon extends CataclysmMob {
    public static final Material CORE_BLOCK_TYPE = Material.CRYING_OBSIDIAN;
    ...
    public PaleParagon(Level level) {
        super(new PaleParagonEntity(level), "Pale Paragon", level);
        super.setSpawnTag(SpawnTag.PERSISTENT);
        super.setListener(new PaleParagonListener(this));

        super.setItem(EquipmentSlot.HEAD, new ItemBuilder(Material.LIGHT).buildAsNMS());
        super.amplifyAttribute(Attributes.SCALE, 1.64);
        super.setAttribute(Attributes.ATTACK_DAMAGE, 1000);
    ...
    protected CataclysmMob createInstance() {
        return new PaleParagon(super.getLevel());
    }

    static class PaleParagonEntity extends IronGolem {
        public PaleParagonEntity(Level level) {
    ...
    static class PaleParagonListener implements Listener {
        private final PaleParagon paragon;

        public PaleParagonListener(PaleParagon paragon) {
        ...
        public void onAttack(EntityDamageByEntityEvent event) {
            ...
            if (player.isBlocking()) player.setCooldown(Material.SHIELD, 60);
        }
