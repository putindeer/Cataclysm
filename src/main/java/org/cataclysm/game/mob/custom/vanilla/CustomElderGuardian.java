...
public class CustomElderGuardian extends CataclysmMob {

    public CustomElderGuardian(Level level) {
        super(new CustomElderGuardianEntity(level), "", level);
    }

    @Override
    protected CataclysmMob createInstance() {
        return new CustomElderGuardian(super.getLevel());
    }

    static class CustomElderGuardianEntity extends ElderGuardian {

        public CustomElderGuardianEntity(Level level) {
            super(EntityType.ELDER_GUARDIAN, level);
            this.persist = false;
