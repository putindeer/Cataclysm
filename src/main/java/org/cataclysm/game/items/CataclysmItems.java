...
public enum CataclysmItems {
    ...
    PALE_BOW(new CataclysmItem(Material.BOW, ItemFamily.PALE_TOOLS).setDisplay("Pale Bow").setDescription("Ex umbris silvae confectum", true).setAbility("Sagitta Mortis", "Dispara una flecha que inflige daño verdadero ignorando defensas, aplica Wither de alto nivel durante 5 segundos, ralentiza, debilita e ilumina a la víctima.").addEnchant(Enchantment.POWER, 20).addEnchant(Enchantment.INFINITY, 1).addEnchant(Enchantment.PUNCH, 2).addEnchant(Enchantment.FLAME, 2).setUnbreakable(true)),
