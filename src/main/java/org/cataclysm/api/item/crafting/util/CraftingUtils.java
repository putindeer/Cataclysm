...
public class CraftingUtils {
    ...
    private static int getMinForCraftings(ItemStack result, List<ItemStack> items, int[] invMatrix){
        List<Integer> howMuchCanCraft = new ArrayList<>();
        for(int i = 0; i < invMatrix.length; i++){
            ItemStack ingredient = items.get(i);
            if(ingredient == null) continue;
            ItemStack ingredientOfResult = getItemOfIngredient(result, ingredient, i);
            if(ingredientOfResult == null) continue;
            int craftingCost = ingredientOfResult.getAmount();
            int howMuchCanCraftForThisItem = (int) Math.floor( (double) ingredient.getAmount() / craftingCost );
            howMuchCanCraft.add(howMuchCanCraftForThisItem);
        }
        return findMin(howMuchCanCraft);
    }
    ...
    public static void removeCustomItemWithShiftClick(CraftingInventory inv, Player target){
        ...
        int howMuchCanCraft = getMinForCraftings(result, items, invMatrixCopy);
        broadcast("Cuantos puedo craftear como minimo: " + howMuchCanCraft);
        for(int i = 0; i < invMatrixCopy.length; i++) {
            ...
            int craftingCost = item.getAmount();
            broadcast("Coste de crafteo: " + craftingCost);
            int howMuchCanAdd = getAmountForAddItem(target, result);
            broadcast("Cuantos puedo agregar: " + howMuchCanAdd);
        ...
        addItem.setAmount(howMuchCanCraft * result.getAmount());
    ...
    private static boolean isResultItemInHashMap(ItemStack result){
        return CustomRecipe.ingredientListWithAmount.contains(result);
    }
    ...
    private static ItemStack getItemOfIngredient(ItemStack result, ItemStack ingredient, int slot){
        Set<Map.Entry<ItemStack, List<Integer>>> entry = CustomRecipe.ingredientsWithAmount.get(result).entrySet();
